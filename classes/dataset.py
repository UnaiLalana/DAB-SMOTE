...
This module provides a basic dataset reader and a simple `dataset` class.

The `dataset` class included here is a minimal structure for storing features,
targets, feature names, and attribute information. The `dataReading` function
reads a dataset file following the specific format expected by this module.

Note
----
The `dataset` class is replaceable by any other class or structure that
provides compatible attributes and can store the dataset in a similar way.
"""
...
def dataReading(fileE):
    ...
    while line.find("@data") < 0:
        if line.find("@attribute") >= 0 and (
            (line.find("real") >= 0) or (line.find("integer") >= 0)
        ):
            line = line.split()
            minVal = line[3].split("[")
